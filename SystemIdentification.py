...
class SI():
	...
	def __init__(self, matlab_api=None, input_data_file=None, degree=1, regularization=0, data=None, form=None, ytrue=None):
		self.ytrue=ytrue
				...
				if name=='ml_regression':
					...
					if name=='robust':
						...
						for j in range(1,9):
							...
							output_loss = np.sum(np.power(np.subtract(output_model.predict(self.combinedIp1[-len(self.combinedIp0)/5:]), self.ytrue[-len(self.combinedIp0)/5:]), 2)) / (len(self.combinedIp0)/5)
						...
						for j in range(1,6):
							i=1 + j/float(10)
							state_model = skl.linear_model.HuberRegressor(epsilon=i, alpha=regular[0], fit_intercept=False) 		#Tolerance ? Max iterations ?
							output_model = skl.linear_model.HuberRegressor(epsilon=i, alpha=regular[1], fit_intercept=False)
							state_model.fit(np.matrix(self.combinedIp0[:-len(self.combinedIp0)/5]), np.matrix(self.regressionData[xdot][:-len(self.regressionData[xdot])/5]))
							output_model.fit(self.combinedIp1[:-len(self.combinedIp0)/5], self.regressionData[y][:-len(self.combinedIp0)/5])
							state_loss = np.sum(np.power(np.subtract(state_model.predict(self.combinedIp0[-len(self.combinedIp0)/5:]), self.regressionData[xdot][-len(self.combinedIp0)/5:]), 2)) / (len(self.combinedIp0)/5)
							output_loss = np.sum(np.power(np.subtract(output_model.predict(self.combinedIp1[-len(self.combinedIp0)/5:]), self.ytrue[-len(self.combinedIp0)/5:]), 2)) / (len(self.combinedIp0)/5)
							loss_table += [(state_loss, output_loss, i)]
						...
						output_model = skl.linear_model.Ridge(alpha=regular[1], fit_intercept=False, copy_X=True)
						hyperparameter_optimization_time=time.time()-hyperparameter_optimization_time
						print('Robust optimization: NS strengths used are {} and hyperparameter_optimization_time={}'.format(regular, hyperparameter_optimization_time))
						...
						if name=='l2':
							...
							if name=='l1':
								...
								hyperparameter_optimization_time=time.time()-hyperparameter_optimization_time
								print('L1 Regularization strengths used are {} and hyperparameter_optimization_time={}'.format(regular, hyperparameter_optimization_time))
							else:
